...
struct ConnectionLinesView: View {
    ...
    var body: some View {
        Canvas { ctx, _ in
            for connection in studio.connections {
                ...
                drawConnection(ctx: ctx, from: fromPt, to: toPt, connection: connection)
            }
        }
    ...
    private func drawConnection(
        ...
        ctx.stroke(path, with: .color(color), style: StrokeStyle(lineWidth: thickness, lineCap: .round))
        ...
        if !connection.label.isEmpty {
            let mid = CGPoint(x: (from.x + to.x) / 2, y: (from.y + to.y) / 2 - 12 * scale)
            ctx.draw(
    ...
    private func drawPortLabel(ctx: GraphicsContext, text: String, near: CGPoint, toward: CGPoint, scale: CGFloat) {
        ...
        // Place label slightly offset from the endpoint toward the midpoint
        let t = 0.15
        let pt = CGPoint(
